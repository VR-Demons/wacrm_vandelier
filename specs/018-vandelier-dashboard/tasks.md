...
- [ ] Identify required environment variables for the integration (e.g. n8n URLs or API keys like `GEMINI_API_KEY`).
- [ ] Use the `coolify-vandelier` MCP server to inject the environment variables into the `vocero-crm` application (UUID: `sdsq2f1w9vr19q8gjyhej8r1`).
- [ ] Ensure `vocero-db` (UUID: `j50x3ripjw61f1euvaut3o9u`) requires no changes.
